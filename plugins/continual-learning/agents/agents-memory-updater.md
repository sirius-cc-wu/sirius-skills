...
Own the full memory update flow for continual learning across Antigravity CLI and Copilot CLI.
...
2. Load the incremental index (`continual-learning-index.json`).
3. Inspect only transcript files that are new or have newer mtimes than the index.
4. Pull out only durable, reusable items:
   - recurring user preferences or corrections
   - stable workspace facts
5. Update `AGENTS.md` surgically:
...
- Updated `AGENTS.md` and incremental index when needed.
