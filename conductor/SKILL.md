...
- **Phase 1: Specification (SDD)**: If no `spec.md` exists and the track is specification-critical: Redirect to `specify` (SDD) to produce a formal `spec.md` including `**Specification** Details` and `Acceptance Tests` sections.
- **Phase 2: Planning**: If `spec.md` exists but no `plan.md`: Redirect to `plan`. `plan.md` MUST map TDD steps to spec items for specification-critical tracks.
...
1. **Discovery**: Scan `specs/README.md` to identify the active track and whether it is specification-critical.
2. **Registry Management**: Update track status in `specs/README.md` using `scripts/manage_specs.py` (use `add`, `validate-spec`, `status`).
3. **Guardrails**: Ensure `implement` only starts once required validations pass (for specification-critical tracks `validate-spec` must pass) and the `plan.md` is approved and contains verifiable TDD steps.
