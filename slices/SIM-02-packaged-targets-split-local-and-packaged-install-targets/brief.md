...
**Slice**: `SIM-02-packaged-targets`  
**Created**: 2026-04-19  
**Status**: Draft  
**Input**: `split-install-modes / SIM-02-packaged-targets`
...
- **Source Story / Increment / Slice**: `SIM-02` / `I2` / `SIM-02-packaged-targets`
...
- **A1**: `SIM-01-local-helper` already introduced a stable local install target surface.
- **A2**: Compatibility behavior for `install` and `uninstall` is still handled in the later `SIM-02-packaged-compat` slice, so this slice should stay focused on naming and target shape.
