...
**Slice**: `SIM-01-local-docs`  
**Created**: 2026-04-19  
**Status**: Draft  
**Input**: `split-install-modes / SIM-01-local-docs`
...
- **Source Story / Increment / Slice**: `SIM-01` / `I1` / `SIM-01-local-docs`
...
3. **Given** a planner reading the installation feature docs, **When** they compare current behavior with the new subfeature direction, **Then** the docs reflect the interim state introduced by `SIM-01-local-helper`.
...
- **A1**: `SIM-01-local-helper` has already landed and introduced working `install-local` / `uninstall-local` targets.
