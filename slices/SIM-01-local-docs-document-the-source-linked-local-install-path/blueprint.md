...
`SIM-01-local-helper`: a new `install-local` / `uninstall-local` source-linked
...
- Constraints:
  - document only the behavior that exists after `SIM-01-local-helper`
...
- Decision: validate by cross-checking the updated docs against the current
  helper behavior and `Makefile` surface already implemented in `SIM-01-local-helper`.
- Result: PASS
