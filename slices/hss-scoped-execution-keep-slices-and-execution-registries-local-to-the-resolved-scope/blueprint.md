...
**Slice**: `hss-scoped-execution-keep-slices-and-execution-registries-local-to-the-resolved-scope`  
**Date**: 2026-04-04  
**Status**: Reviewed
**Spec**: `brief.md`
...
execution context. Parent config inheritance from hss-config-inheritance stays
...
- Constraints:
  ...
  - rely on inherited execution config from hss-config-inheritance when child
...
- Target files:
  - `skills/guide-execution/scripts/manage_execution.py`
  - `skills/guide-execution/tests/test_manage_execution.py`
- Dependencies: hss-config-inheritance
- Steps:
...
- Rationale: HSS-03 and hss-config-inheritance already established the
...
- Handoff notes for implementation: keep the new helpers additive and localized
  so hss-guide-scope can later document execution handoff without revisiting
...
- Durable artifact note:
  - hss-scoped-execution now makes guide-execution resolve config and registry
