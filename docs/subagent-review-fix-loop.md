...
component "Reviewer\nread-only\nreturns full report" as Reviewer #FFFBEA
component "Worker\nfixer role\napplies required fixes" as Fixer #EEF8EE
...
The example names its write-enabled agent `worker`. Use `worker` for the fixer
role, or create a separate `fixer.md` agent definition. The sample agent files
can name models that are not available in the current Pi installation. Remove
a `model:` field to inherit the controller's active model and thinking level,
or set it to an available `provider/model` value.
...
2. Start a fresh reviewer subagent. The reviewer is read-only.
...
5. Pass only unresolved must-fix findings to a fresh `worker` subagent acting
   as the fixer.
6. The fixer may change only the authorized scope. It must run relevant
   validation and report each required ID as fixed, not fixed, or disputed.
7. Start a fresh reviewer after every fix. It must verify previous required
   IDs and review the complete updated change for regressions and new findings.
...
```json
{
  "chain": [
    {
      "agent": "reviewer",
      "task": "Review the complete bounded change. Separate must-fix and non-blocking findings."
    },
    {
      "agent": "worker",
    ...
    {
      "agent": "reviewer",
      "task": "Start a fresh review of the complete updated change. Verify the worker dispositions below and report any remaining or new must-fix findings.\n\n{previous}"
    }
  ]
}
```

The fixed chain passes the full first review to the worker. It can restrict
which findings the worker acts on, but it does not strictly restrict which
findings the worker receives. Use separate controller-managed calls when the
handoff itself must contain only required findings.

The final reviewer receives only the worker's output through `{previous}`. The
worker must therefore preserve the required finding IDs and dispositions. The
