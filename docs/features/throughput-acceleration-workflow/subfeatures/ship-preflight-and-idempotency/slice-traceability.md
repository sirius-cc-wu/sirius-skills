...
| SPI-02 | M | Add optional preflight before mutation-capable `ship` operations | I1 | spi-preflight-contract | `.skills/execution.json`, `skills/ship/` | taw-ship-backlog-integration, taw-runtime-foundation |  | Establish config and readiness shape before enforcing blocks |
| SPI-02 | M | Add optional preflight before mutation-capable `ship` operations | I2 | spi-mutation-gating | `skills/ship/`, `skills/ship-slice/` | spi-preflight-contract, taw-ship-slice-loop |  | Uses existing approval and commit guardrails as canonical blocker reasons |
