...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| taw-runtime-foundation | TAW-03 | Add shared accelerator runtime support | Introduce `lib/workflow_runtime`, runtime file-locking helpers, checkpoint/event-log/learnings stores, and packaged runtime sync for consuming skills. | new `lib/workflow_runtime/`, `scripts/sync_shared_skill_runtime.py`, packaging tests | primary | `pytest -q tests/test_install_target_modes.py skills/ship/tests/test_ship.py -k runtime` | create slice |  | yes |
