...
- Produce planning outputs that are concrete enough for later review and track bootstrap.

## Non-Goals

- Manage task-scoped execution artifacts such as `brief.md` or `plan.md`.
...
- Task tracker owner receiving execution-ready work items.
...
- `discover`, `design`, `ui-flow`, `breakdown`, `review-planning`, and `track` must stay feature-scoped.
- Increments belong in planning artifacts, not execution-track state.
...
- `skills/breakdown/SKILL.md` and its templates define `task-planning.md` and `task-traceability.md`.
...
- Breakdown outputs produce execution-ready slices and demonstrable increments without inventing execution-track state.
