...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| WSC-01 | M | Run high-confidence consistency checks around important transitions | I2 | WSC-01-transition-guardrails | Planning, subfeature, and execution transition owners | WSC-03-semantic-preview | WSC-01-transition-guardrails | Reuses the same normalized findings shape established by the preview path before owner scripts warn or block. |
| WSC-02 | M | Share one reconciliation model across maintenance workflows | I1 | WSC-02-shared-library | Shared library, Audit, Trace |  | WSC-02-shared-library | First execution packet establishes the canonical shared workflow-state library and normalized traceability/reconciliation primitives. |
