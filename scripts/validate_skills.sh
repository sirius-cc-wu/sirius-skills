...
relationship_guide="$root/catalog/skill-relationships.md"
relationship_svg="$root/catalog/skill-relationships.svg"
...
test -f "$relationship_guide" || fail "missing $relationship_guide"
test -f "$relationship_svg" || fail "missing $relationship_svg"
...
architecture_skill="$root/skills/design-software-architecture/SKILL.md"
architecture_metadata="$root/skills/design-software-architecture/agents/openai.yaml"
...
grep -q "^## Support-Envelope Gate$" "$development_skill" || fail "iterative development skill missing support-envelope gate"
grep -q "dynamically extensible population" "$development_skill" || fail "iterative development skill missing open support-population safeguard"
grep -q "cross-mode" "$development_skill" || fail "iterative development skill missing cross-mode support safeguard"
...
test -f "$architecture_metadata" || fail "design-software-architecture missing agents/openai.yaml"
grep -Fq '$design-software-architecture' "$architecture_metadata" || fail "software architecture metadata missing skill invocation"
grep -q "^## View Selection$" "$architecture_skill" || fail "software architecture skill missing view selection"
grep -q "measurable quality-attribute scenarios" "$architecture_skill" || fail "software architecture skill missing quality scenarios"
grep -q "every C4 level" "$architecture_skill" || fail "software architecture skill missing minimal-view safeguard"
...
template_types=(
  ...
  "design-software-architecture|Software Architecture Design"
...
grep -q '^@startuml sirius-skills-birds-eye$' "$relationship_guide" || fail "relationship guide missing embedded overview PlantUML"
test "$(grep -c '^```plantuml$' "$relationship_guide")" -eq 4 || fail "relationship guide must retain four embedded PlantUML views"
grep -q '<svg ' "$relationship_svg" || fail "relationship overview is not SVG"
grep -Fq "catalog/skill-relationships.svg" "$root/README.md" || fail "README missing relationship overview"
