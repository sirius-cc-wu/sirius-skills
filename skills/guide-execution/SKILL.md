...
4. Route slice-scoped work to `blueprint`, optional legacy `brief`, active repository implementation, `review-execution`, or `close-slice`.
5. Update slice readiness status when a phase is complete.

`guide-execution` owns orchestration and readiness only. It should not absorb artifact authoring that belongs to `blueprint`, optional legacy `brief`, `review-execution`, or `close-slice`.
...
- `draft`
- `brief_ready` (legacy standalone-brief path only)
- `blueprint_ready`
- `execution_ready`
- `closed`
