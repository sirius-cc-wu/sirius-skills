...
SHIP_SCRIPT = SKILLS_DIR / "ship" / "scripts" / "ship.py"
GUIDE_EXECUTION_SCRIPT = SKILLS_DIR / "guide-execution" / "scripts" / "manage_execution.py"
...
def resolve_target(repo_root: Path, selector: str, *, explicit_scope: Optional[str]) -> Dict[str, Any]:
    command = ["python3", str(SHIP_SCRIPT), selector, "--json"]
    if explicit_scope:
...
def run_ship_operation(
    ...
    *,
    ...
    command = ["python3", str(SHIP_SCRIPT), selector, f"--{operation}", "--json"]
    if explicit_scope:
        command.extend(["--scope", explicit_scope])
