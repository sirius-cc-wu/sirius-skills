---
name: track
description: Bootstraps a task-scoped execution track from a ready work item and hands off to execution-driver.
---
...
4. Hand off to `execution-driver` for `brief.md` and `plan.md`.
...
- Keep task execution state in the tracker and track readiness in `execution-driver`.
...
```bash
python3 skills/execution-driver/scripts/manage_execution.py add "<track-id>" "<feature-name>"
```
...
4. Hand off to `execution-driver` to author `brief.md` and `plan.md`.
